...
const Properties = () => {
  ...
  const fetchProperties = async () => {
    try {
      const response = await axios.get(
        "https://rentify-backend-de1n.onrender.com/api/properties"
      );
  ...
  const handleFilter = async () => {
    try {
      ...
      const response = await axios.get(
        "https://rentify-backend-de1n.onrender.com/api/filter",
        {
          params,
        }
      );
  ...
  const getContactDetails = async (ownerId) => {
    try {
      const response = await axios.get(
        `https://rentify-backend-de1n.onrender.com/api/users/${ownerId}`
      );
  ...
  const handleInterestedClick = async (property) => {
    if (!isLoggedIn) {
      ...
      if (contactData) {
        ...
        try {
          await axios.post(
            `https://rentify-backend-de1n.onrender.com/api/properties/${user.email}`,
            {
              name: `${contactData.firstName} ${contactData.lastName}`,
              email: contactData.email,
              phone: contactData.phone,
  ...
  const handleLike = async (propertyId) => {
    try {
      ...
      if (liked) {
        ...
        if (confirmUnLike) {
          await axios.delete(
            `https://rentify-backend-de1n.onrender.com/api/likes/deleteLike/${propertyId}/${userId}`,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
              },
        ...
        await axios.post(
          "https://rentify-backend-de1n.onrender.com/api/likes/addLike",
          {
            userId,
            propertyId,
          },
  ...
  const getLikeCount = async (propertyId) => {
    try {
      const response = await axios.get(
        `https://rentify-backend-de1n.onrender.com/api/likes/likeCount/${propertyId}`
      );
  ...
  const isPropertyLiked = async (propertyId) => {
    try {
      const userId = user._id;
      const response = await axios.get(
        `https://rentify-backend-de1n.onrender.com/api/likes/isLiked/${propertyId}/${userId}`
      );
        ...
        {currentItems.map((property) => (
          <li key={property._id} className="property-item">
            <div className="property-info">
              <img
                src={`https://rentify-backend-de1n.onrender.com/uploads/${property.photo}`}
